...
public class ConsultasVenta {
    ...
    public String realizarVenta(String idCliente, List<String> productos, List<Integer> cantidades) {
        ...
        try {
            ...
            // Consolidar productos (agrupar repetidos)
            ...
            Collections.sort(idsOrdenados); // Prevenir Deadlocks

            Map<String, Double> precios = new HashMap<>();

            // 1. Validar Stock y Precios usando la constante SQL
            for (String idProd : idsOrdenados) {
                int cantidadNecesaria = productosConsolidados.get(idProd);
                try (PreparedStatement ps = con.prepareStatement(ConsultasSQL.VTA_LOCK)) {
            ...
            try (PreparedStatement psVenta = con.prepareStatement(ConsultasSQL.VTA_INSERT)) {
            ...
            for (String idProd : idsOrdenados) {
                ...
                try (PreparedStatement psDet = con.prepareStatement(ConsultasSQL.VTA_INSERT_DET)) {
                ...
                try (PreparedStatement psStock = con.prepareStatement(ConsultasSQL.VTA_UPD_STOCK)) {
    ...
    public DefaultTableModel leerVentas() {
        ...
        try (Connection con = Conexion.getConexion(); 
             ...
             ResultSet rs = st.executeQuery(ConsultasSQL.VTA_SELECT_ALL)) {
            while (rs.next()) {
    ...
    public DefaultTableModel leerDetalles(String idVenta) {
        ...
        DefaultTableModel modelo = new DefaultTableModel(null, col);
        try (Connection con = Conexion.getConexion(); 
             PreparedStatement ps = con.prepareStatement(ConsultasSQL.VTA_DETALLES)) {
    ...
    public boolean eliminarVenta(String idVenta) {
        try (Connection con = Conexion.getConexion()) {
            con.setAutoCommit(false);
            try (PreparedStatement ps1 = con.prepareStatement(ConsultasSQL.VTA_DEL_DET);
                 PreparedStatement ps2 = con.prepareStatement(ConsultasSQL.VTA_DEL_VTA)) {
